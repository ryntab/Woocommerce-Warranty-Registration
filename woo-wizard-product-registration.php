...
function order_my_custom()
{
    ...
    $claimedDate = get_post_meta($order->get_id(), 'claimed_date', true);
    $claimedTime = get_post_meta($order->get_id(), 'claimed_time', true);
    $registrationDate = get_post_meta($order->get_id(), 'registration_date', true);
    $registrationTime = get_post_meta($order->get_id(), 'registration_time', true);
    $registrationSerial = get_post_meta($order->get_id(), 'registration_serial', true);
...
{
    global $wpdb;
    $serial = strval($_REQUEST['keyword']);
    $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM `wp_postmeta` WHERE `meta_key` = 'registration_serial' AND `meta_value` = '$serial'"));
